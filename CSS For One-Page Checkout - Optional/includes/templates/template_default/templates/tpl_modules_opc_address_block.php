...
?>
<!-- bof 1 of 3 updated for WCAG --> 
    <div id="choices-<?= $which ?>"><?= zen_draw_pull_down_menu(
    "address-$which",
    $address_selections,
    $selected,
    'aria-label="' . (($which === 'bill') ? 'Billing Address' : 'Shipping Address') . '"'
) ?></div>
<!-- eof  1 of 3 updated for WCAG --> 
<?php
    }
}
...
?>
<!-- bof 2 of 3 updated for WCAG -->   
<label class="inputLabel" for="<?= $state_zone_id ?>">
    <?= ENTRY_STATE ?>
</label>
<!-- eof 2 of 3 updated for WCAG -->  
<?php
...
// bof 3 of 3 updated for WCAG //
    echo $_SESSION['opc']->formatAddressElement($which, 'state',  $address['state'], ENTRY_STATE, TABLE_ADDRESS_BOOK, 'entry_state', ENTRY_STATE_MIN_LENGTH, ENTRY_STATE_TEXT) . $clear_both . '<br>';
// eof 3 of 3 updated for WCAG //
}
